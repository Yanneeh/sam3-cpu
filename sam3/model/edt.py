...
def _edt_cpu(data: torch.Tensor):
    import cv2
    ...
    for sample in data_np:
        sample_uint8 = sample.astype("uint8")
        result.append(cv2.distanceTransform(sample_uint8, cv2.DIST_L2, 0))
    return torch.from_numpy(np.stack(result, axis=0)).to(device=device, dtype=torch.float32)
